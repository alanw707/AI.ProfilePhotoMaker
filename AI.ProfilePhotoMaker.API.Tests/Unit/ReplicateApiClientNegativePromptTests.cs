...
public class ReplicateApiClientNegativePromptTests
{
    ...
    [Fact]
    public async Task GenerateImagesAsync_FallsBackToLinkedin_WhenRequestedStyleNotFound()
    {
        // F3 fix: verify fallback path uses "linkedin" when requested style is missing from DB
        var httpHandlerMock = new Mock<HttpMessageHandler>(MockBehavior.Strict);
        HttpRequestMessage? capturedRequest = null;
        ...
        // Only seed "linkedin" — requested style "nonexistent" will not be found,
        // triggering the fallback path that should resolve to linkedin
        db.Styles.Add(new Style
        {
            Id = 1100,
            Name = "linkedin",
            Description = "LinkedIn fallback style",
            PromptTemplate = "A photo of {subject}, linkedin fallback portrait of {gender} {ethnicity}",
            NegativePromptTemplate = "linkedin-specific-negative-term",
            IsActive = true,
            CreatedAt = DateTime.UtcNow,
            UpdatedAt = DateTime.UtcNow
        });
        ...
        var input = document.RootElement.GetProperty("input");
        Assert.True(input.TryGetProperty("negative_prompt", out var negativePrompt));
        var negativePromptValue = negativePrompt.GetString() ?? string.Empty;
