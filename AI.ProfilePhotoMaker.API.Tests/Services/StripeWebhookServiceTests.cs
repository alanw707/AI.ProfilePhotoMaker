...
public class StripeWebhookServiceTests
{
    ...
    public async Task HandleEventAsync_PaymentIntentSucceeded_InactivePackageThrowsForWebhookRetry()
    {
        ...
        Assert.Equal(PaymentStatus.Succeeded, updatedTransaction.Status);
        Assert.Equal(StripeWebhookOperationStatus.Failed,
            (await context.StripeWebhookOperations.AsNoTracking().SingleAsync()).Status);
