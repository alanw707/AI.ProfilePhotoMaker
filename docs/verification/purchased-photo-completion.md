...
A paid replay remained prohibited. Temporary safe headshot telemetry was enabled by `Diagnostics__CaptureHeadshotFailure=true`; it captured at most one HTTP 400 rejection per container process, used a server-generated correlation header, and excluded user IDs, emails, cookies, authorization headers, storage paths/URLs, image data, prompts, and payment identifiers. The gate remains regression-covered by `HeadshotsControllerTelemetryTests.Generate_CapturesOneSafeFailureCorrelationWhenTemporaryTelemetryIsEnabled`, but the deployment configuration now removes it because the evidence-bearing response below was recovered without a paid retry.

### Captured generation 400 and root cause

The redacted production response captured on 2026-08-31 is an MVC validation rejection from `/api/enhancement/enhance`, not `/api/headshots/generate`: `400` with `ImageUrl: Image URL must be a valid URL`. Its cause is deterministic: the legacy enhancement client sent both a relative display URL and its valid storage path, while the DTO's `[Url]` attribute rejected the relative URL before the controller could use the storage path. This happens before the controller, provider call, or allowance consumption.

The regression is red-capable in `photo-enhancement.component.spec.ts`: before the fix it observes the invalid relative `imageUrl` alongside `imageStoragePath`; it now requires storage-path-only requests. `EnhancePhotoRequestDtoTests.Validate_AllowsStoragePathWhenLegacyDisplayUrlIsRelative` also keeps the API validation contract aligned for legacy clients. The saved January legacy-credit artifact remains a separate comparator, not evidence for this validation failure.
