...
The first new fixture parser failed against the real .NET multipart request and the fixture SQL seed initially lacked `QUOTED_IDENTIFIER`; both failures were corrected without weakening the assertions. The final chain check passed, followed by complete 490 API, 517 UI (two existing skips), 21 browser and production build/lint gates. The chain documentation and test harness deployed in `9363dd7` through workflow `34071563861`; the new duplicate-action correction below still requires its own complete release gates and deployment.

## Duplicate-action audit correction

The final auditor identified that the earlier browser lifecycle check asserted one submission after a single action and hide/reopen, but did not attempt a second action. The processing browser suite now issues two genuine `page.mouse.dblclick` pointer clicks while the first API request is deliberately held pending. It verifies exactly one request is observed, a loading dialog remains visible, and unknown-outcome recovery persists. This runs for both candidate generation and Straighter posture refinement at 390px and 1440px (four red-capable browser checks). The test would catch removal of the processing/re-entry guard; it is not a mocked component call.

The focus-restoration path was also made post-render (`requestAnimationFrame`) and gives an error heading priority on failure and a result heading priority on success. This preserves the existing keyboard behavior even after pointer double-clicks. Focused duplicate gate: `/tmp/aipm-duplicate-action-green.txt` (12 checks including existing modal lifecycle). Full release gates and deployment remain pending.
