...
#
# This script would have detected the Azure Storage configuration mismatch
...
check_production_requirements() {
    ...
    # Azure Storage is critical for production
    if [[ -z "${AZURE_STORAGE_CONNECTION_STRING:-}" ]]; then
        log_critical "Azure Storage connection string missing in production environment"
        add_remediation "Set AZURE_STORAGE_CONNECTION_STRING for production deployment"
    fi
    
    if [[ -z "${AZURE_STORAGE_CONTAINER_NAME:-}" ]]; then
        log_critical "Azure Storage container name missing in production environment"
        add_remediation "Set AZURE_STORAGE_CONTAINER_NAME for production deployment"
    fi
}
...
detect_naming_mismatches() {
    ...
    # Common mismatches that have caused issues
    ...
    # Look for storage-related variables that don't match expected names
    local storage_vars=$(echo "$BICEP_ENV_VARS$BICEP_CONFIG_VARS" | grep -i storage | grep -v "AZURE_STORAGE_CONNECTION_STRING\|AZURE_STORAGE_CONTAINER_NAME" || true)
    
    if [[ -n "$storage_vars" ]]; then
        echo "$storage_vars" | while read -r var; do
            [[ -n "$var" ]] && log_warning "Potential storage variable mismatch: $var (should it be AZURE_STORAGE_*?)"
        done
    fi
}
...
check_production_staging_drift() {
    ...
    # Azure Storage is REQUIRED in production/staging
    if ! echo "$REQUIRED_APP_VARS" | grep -q "AZURE_STORAGE_CONNECTION_STRING"; then
        log_critical "Application doesn't mark Azure Storage as required for production"
        add_remediation "Review EnvironmentConfiguration.cs for production Azure Storage requirements"
    fi
