...
1. **Add method to find images approaching deletion**
   - [x] Add `GetImagesApproachingDeletionAsync(int daysBeforeDeletion, int windowSizeDays = 1)` method to `IRetentionPolicyService` interface
   - [x] Implement in `RetentionPolicyService` to query images where `ScheduledDeletionDate` is between `now + daysBeforeDeletion - windowSizeDays/2` and `now + daysBeforeDeletion + windowSizeDays/2` (configurable window, default 1 day)
   - [x] Include `UserProfile` and `ApplicationUser` in query to get email addresses
   - [x] Return grouped results by user (userId, email, list of images)

2. **Add email notification method**
   - [x] Add `SendRetentionDeletionWarningAsync(string userId, string? email, int imageCount, DateTime deletionDate, int daysUntilDeletion)` to `IEmailNotificationService` interface
   - [x] Implement in `EmailNotificationService` with email template
   - [x] Email template includes: polite subject (e.g., "Reminder: Your photos will be deleted in 7 days" for 7-day, "Your photos will be deleted in 14 days" for 14-day), image count, deletion date (formatted), link to gallery, polite and clear messaging
   - [x] Use existing `SendEmailAsync` infrastructure with template tag "retention-deletion-warning-{daysUntilDeletion}d" (e.g., "retention-deletion-warning-14d", "retention-deletion-warning-7d")
   - [x] Handle null/empty email gracefully (log and return without sending)
   - [x] Email tone should be polite, professional, and respectful

3. **Integrate notification into background service**
   - [x] Update `RetentionPolicyBackgroundService.PerformRetentionPolicyCheck()` to call notification logic for both notification windows
   - [x] After `SetRetentionDatesForExistingImagesAsync()`, check for images approaching deletion at both windows:
     - 14 days before deletion (check 13-15 day window)
     - 7 days before deletion (check 6-8 day window)
   - [x] For each notification window, group images by user (userId)
   - [x] Track sent notifications in-memory per window (Dictionary<int, HashSet<string>>) to prevent duplicates within same run
   - [x] Send one email per user per notification window with aggregated image count
   - [x] Log notification results (sent count per window, skipped count, errors)
   - [x] Continue with existing deletion logic after notifications
...
4. **Add configuration for notification timing (optional)**
   ...
   - [x] Can be added later if needed; hardcode to [14, 7] for initial implementation

5. **Tests**
   - [x] Unit test: `GetImagesApproachingDeletionAsync` returns correct images in date window for both 14-day and 7-day windows
   - [x] Unit test: `SendRetentionDeletionWarningAsync` sends email with correct content and polite tone for each notification stage
...
- [x] Background service identifies images scheduled for deletion at both 14-day and 7-day windows
- [x] Users receive 14-day warning email when they have images scheduled for deletion in approximately 14 days
- [x] Users receive 7-day warning email when they have images scheduled for deletion in approximately 7 days
- [x] Users receive one email per notification window per background service execution (no duplicates within same window in same run)
- [x] Email includes: image count, deletion date (formatted), link to gallery, polite and professional messaging
- [x] Email tone is polite, clear, and respectful
- [x] No duplicate emails sent to same user within single background service execution for same notification window
- [x] Users without email addresses are skipped (logged, not errored)
- [x] Email send failures are logged but don't prevent retention deletion from proceeding
- [x] Existing retention deletion functionality continues to work unchanged
- [x] Notifications sent at appropriate times (14 days and 7 days before scheduled deletion)
- [x] Email content is clear, professional, polite, and actionable
