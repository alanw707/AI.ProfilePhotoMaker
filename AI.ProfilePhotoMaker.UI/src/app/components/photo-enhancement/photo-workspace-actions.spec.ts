...
describe('Photo workspace action recovery', () => {
  ...
  beforeEach(() => {
    ...
    spyOn(component, 'canDownloadPackage').and.returnValue(true);
    spyOn(component, 'canApplyPremiumAugmentation').and.callFake(
      () => !component.isApplyingPremiumAugmentation
    );
  });

  it('exposes the premium augmentations (keep-main rich design)', () => {
  ...
  for (const packageCode of ['pro_package', null]) {
    it(`restores paid work without preview promotion (${packageCode ?? 'no remaining allowance'})`, async () => {
      ...
      Reflect.set(component, 'loadAuthorizedCandidateImages', async (candidates: unknown[]) => candidates);
      Reflect.set(component, 'clearInterruptedGeneration', () => undefined);
      ...
      expect(Reflect.get(component, 'hasConfirmedPaidGenerationForCurrentResult')).toBeTrue();
