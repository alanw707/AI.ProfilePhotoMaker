...
describe('FileUploadSectionComponent', () => {
  ...
  beforeEach(async () => {
    const fileUploadServiceSpy = jasmine.createSpyObj('FileUploadService', ['uploadImages', 'deleteImage']);
    const faceDetectionServiceSpy = jasmine.createSpyObj('FaceDetectionService', ['validateImage']);
    const fileUploadManagerServiceSpy = jasmine.createSpyObj('FileUploadManagerService', ['processFiles']);
    ...
    mockFaceDetectionService = TestBed.inject(FaceDetectionService) as jasmine.SpyObj<FaceDetectionService>;
    mockFileUploadManagerService = TestBed.inject(FileUploadManagerService) as jasmine.SpyObj<FileUploadManagerService>;
  ...
  describe('File Validation', () => {
    ...
    it('should validate file size correctly', () => {
      const validFile = new File(['content'], 'test.jpg', { type: 'image/jpeg' });
      const largeFile = new File(['x'.repeat(8 * 1024 * 1024)], 'large.jpg', { type: 'image/jpeg' });
  ...
  describe('Quality Validation', () => {
    it('should validate image quality successfully', fakeAsync(() => {
      ...
      spyOn(component, 'getImageDimensions' as any).and.returnValue(Promise.resolve({ width: 800, height: 600 }));
    ...
    it('should reject images with insufficient resolution', fakeAsync(() => {
      const mockFile = new File(['content'], 'test.jpg', { type: 'image/jpeg' });
      spyOn(component, 'getImageDimensions' as any).and.returnValue(Promise.resolve({ width: 400, height: 300 }));
      
      component['validateImageQuality']([mockFile]);
      tick();
      
      expect(component.qualityCheckErrors.length).toBe(1);
      expect(component.qualityCheckErrors[0].errors[0]).toContain('resolution 400x300 is too small');
    }));
  });
  ...
  describe('File Management', () => {
    it('should remove file correctly', () => {
      ...
      component.selectedFilesWithQuality = [{ file: mockFile, isValid: true } as SelectedFileWithQuality];
  ...
  describe('Image Deletion', () => {
    it('should delete uploaded image successfully', () => {
      ...
      expect(component.uploadedImageDeleted.emit).toHaveBeenCalledWith({ thumb: mockThumb, index: 0 });
      expect(mockNotificationService.success).toHaveBeenCalledWith('Deleted', 'Image deleted successfully');
    });
    ...
    it('should handle delete error with 404 status', () => {
      ...
      expect(mockNotificationService.error).toHaveBeenCalledWith('Error', jasmine.stringContaining('not found'));
    ...
    it('should handle invalid thumbnail data', () => {
      ...
      expect(mockNotificationService.error).toHaveBeenCalledWith('Error', jasmine.stringContaining('missing or invalid'));
    });
  });
  ...
  describe('Upload Process', () => {
    it('should upload images successfully', () => {
      ...
      const mockUploadResponse = { 
        response: { uploadedFiles: [{ id: 1, url: 'uploaded.jpg' }] } 
      };
      ...
      expect(component.uploadCompleted.emit).toHaveBeenCalledWith(mockUploadResponse.response.uploadedFiles);
      expect(mockNotificationService.success).toHaveBeenCalled();
    });
    ...
    it('should handle upload error', () => {
      ...
      expect(mockNotificationService.error).toHaveBeenCalledWith('Upload Error', 'An error occurred during upload');
      expect(component.isUploading).toBeFalse();
    });
    ...
    it('should not upload if no files selected', () => {
      ...
      expect(mockNotificationService.error).toHaveBeenCalledWith('No Files Selected', 'Please select files to upload');
    });
  });
  ...
  describe('Helper Methods', () => {
    ...
    it('should get invalid files count', () => {
      component.qualityCheckErrors = [
        {} as QualityCheckError,
        {} as QualityCheckError
      ];
    ...
    it('should format compact error messages', () => {
      const longMessage = 'No face detected in image. Please upload a clear photo with your face visible.';
      ...
      expect(component.getCompactErrorMessage(veryLongMessage)).toBe('This is a very long message that sho...');
    });
  });
