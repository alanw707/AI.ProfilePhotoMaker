...
import { WorkspaceState, IWorkspaceStateService } from '../interfaces/service.interfaces';

/**
 * Orchestrates multiple specialized services to provide a unified dashboard interface
 * Replaces the monolithic WorkspaceStateService with a coordination-focused approach
 */
@Injectable({
  providedIn: 'root',
})
export class WorkspaceStateService implements IWorkspaceStateService {
  private readonly CACHE_KEY = 'workspace_state_data';
  ...
  // Combined workspace state observable
  public readonly state$: Observable<WorkspaceState>;
  ...
  /**
   * Get current combined workspace state
   */
    ...
    const imageUpdate = imageFields.reduce(
      (acc, field) => {
        if (field in newState) {
          acc[field] = newState[field as keyof WorkspaceState];
        }
        return acc;
      },
  ...
  /**
   * Load complete workspace data by orchestrating specialized services
   */
  async loadInitialWorkspaceData(): Promise<void> {
    ...
    try {
      ...
      if (failedLoads === results.length) {
        throw new Error('All workspace services failed to load');
      } else if (failedLoads > 0) {
        console.log(
          `✅ Workspace loaded with ${results.length - failedLoads}/${results.length} services successful`
        );
      }
      ...
      console.error('❌ Workspace state load failed:', error);
      // Try to load cached data as fallback
      this.loadCachedDataAsFallback();
      this._notificationService.error(
        'Workspace Load Failed',
        'Could not load workspace data. Please refresh the page or try again.'
      );
    } finally {
  ...
  private loadCachedDataAsFallback(): void {
    try {
      ...
      if (cachedData) {
        console.log('🔄 Loading cached workspace data as fallback');
      ...
      console.warn('⚠️ Could not load cached workspace data:', error);
    }
  }
  ...
  private loadRemainingDataAsync(): void {
    forkJoin({
      ...
      error: error => {
        console.error('Failed to load additional workspace data:', error);
        const currentState = this._coordinatorState.getValue();
